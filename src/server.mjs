...
function runtimeSourceCommit() {
  const configured = String(process.env.SOURCE_COMMIT || "").trim();
  if (/^[0-9a-f]{40}$/.test(configured)) return configured;
  try {
    const match = readFileSync(".source-commit.env", "utf8").match(/^SOURCE_COMMIT=([0-9a-f]{40})$/m);
    return match?.[1] || "";
  } catch {
    return "";
  }
...
async function healthPayload(config) {
  const payload = {
    ...
    capabilities: ["file_read", "patch_preview", "patch_apply", "pull_request_read", "pull_request_ready", "pull_request_merge", "github_rest", "github_graphql", "github_repository_search", "github_code_search", "github_repository_create", "github_app_diagnose", "repository_diagnose", "ref_write_probe", "actions_job_logs"],
  ...
  try {
    const probeRepository = config.allowedRepos[0];
    const auth = await checkGitHubAuth(probeRepository ? { repository_full_name: probeRepository } : {}, config);
    payload.github_auth = { status: auth.status };
  } catch (error) {
    payload.github_auth = {
