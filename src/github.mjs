...
export async function githubRestRequest(payload, config) {
  ...
  if (paginate) {
    const boundedPerPage = Math.min(maxItemsRaw, 100);
    const requestedPerPage = Number(payload.query?.per_page);
    if (payload.query?.per_page == null || (Number.isInteger(requestedPerPage) && requestedPerPage > boundedPerPage)) {
      payload.query = { ...(payload.query || {}), per_page: boundedPerPage };
    }
  }
...
export async function githubGraphqlRequest(payload, config) {
  ...
  const scopeMode = String(config.githubRepositoryScopeMode || "token").toLowerCase();
  if (scopeMode !== "token" && config.allowedRepos.length === 0) {
  ...
  if (scopeMode !== "token" && config.allowedRepos.length > 0 && !repositoryScope) {
  ...
  if (scopeMode !== "token" && config.allowedRepos.length > 0 && authMode === "user") {
